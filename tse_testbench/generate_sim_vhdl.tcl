...
if {[lsearch -regexp $deviceDieList {^HSSI_GDR}] != -1} {
  if {[is_project_open]} {
     ...
     if {[string compare $project_name "generate_sim_vhdl"] != 0} {
        ...
        post_message -type error "and open the project \"generate_sim_vhdl\""
        return 1
     }
  }
} else {
...
if { [string match "QSYS_PRO" $is_qsys_edition] } {
   ...
   puts $temp
   if {[lsearch -regexp $deviceDieList {^HSSI_GDR}] != -1} {
     catch {eval [concat [list exec "qsys-generate" --simulation=VHDL --synthesis=VHDL] "$output_dir/${variant_name}_sim/${variant_name}.ip"]} temp
     catch {eval [concat [list exec quartus_tlg --verbose generate_sim_vhdl]]} temp
   } else {
     catch {eval [concat [list exec "qsys-generate" --simulation=VHDL] "$output_dir/${variant_name}_sim/${variant_name}.ip"]} temp
   }
   puts $temp
   ...
   if {[lsearch -regexp $deviceDieList {^HSSI_GDR}] != -1} {
     catch {eval [concat [list exec ip-setup-simulation --quartus-project=generate_sim_vhdl.qpf --output-directory=$output_dir/$variant_name --use-relative-paths=true]]}
   } else {
