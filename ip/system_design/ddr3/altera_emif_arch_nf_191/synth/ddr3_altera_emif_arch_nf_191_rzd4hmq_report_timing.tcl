...
if { ! [timing_netlist_exist] } {
   ...
   source "$script_dir/ddr3_altera_emif_arch_nf_191_rzd4hmq_report_timing_core.tcl"
   if { ! [timing_netlist_exist] } {
...
ddr3_altera_emif_arch_nf_191_rzd4hmq_initialize_ddr_db ddr_db var
...
foreach inst $instances {
   ...
   if {[llength $instances] <= 1} {
      set fbasename "${::GLOBAL_ddr3_altera_emif_arch_nf_191_rzd4hmq_corename}"
   } else {
      set fbasename "${::GLOBAL_ddr3_altera_emif_arch_nf_191_rzd4hmq_corename}_${inst_id}"
   }
   ...
   ddr3_altera_emif_arch_nf_191_rzd4hmq_perform_read_capture_analysis $opcname $inst pins var summary

   #######################################
   # Write Analysis

   ddr3_altera_emif_arch_nf_191_rzd4hmq_perform_write_launch_analysis $opcname $inst pins var summary

   #######################################
   # Address/command Analyses

   ddr3_altera_emif_arch_nf_191_rzd4hmq_perform_ac_analysis  $opcname $inst pins var summary

   #######################################
   # DQS Gating Analysis

   ddr3_altera_emif_arch_nf_191_rzd4hmq_perform_dqs_gating_analysis   $opcname $inst pins var summary

   #######################################
   # Write Levelling Analysis

   ddr3_altera_emif_arch_nf_191_rzd4hmq_perform_write_levelling_analysis $opcname $inst pins var summary

   #######################################
   # Analyses related to FPGA core

   ddr3_altera_emif_arch_nf_191_rzd4hmq_perform_core_analysis $opcname $inst pins var summary

   #######################################
   # Print out the Summary Panel for this instance

   set summary [lsort -command ddr3_altera_emif_arch_nf_191_rzd4hmq_sort_proc $summary]

   set f -1
   set fname "${fbasename}_summary.csv"

   if { [ddr3_altera_emif_arch_nf_191_rzd4hmq_get_operating_conditions_number] == 0 } {
      set f [open $fname w]

      puts $f "Core: ${::GLOBAL_ddr3_altera_emif_arch_nf_191_rzd4hmq_corename} - Instance: $inst"
   ...
   post_message -type info "Core: ${::GLOBAL_ddr3_altera_emif_arch_nf_191_rzd4hmq_corename} - Instance: $inst"
   ...
   foreach summary_line $summary {
      ...
      if {$su != "--"} {
         set su [ ddr3_altera_emif_arch_nf_191_rzd4hmq_round_3dp $su]
      }
      if {$hold != "--"} {
         set hold [ ddr3_altera_emif_arch_nf_191_rzd4hmq_round_3dp $hold]
      }
